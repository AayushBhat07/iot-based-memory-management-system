  ...
  return <div className="min-h-screen bg-gradient-to-b from-background to-secondary">
      <header className="container mx-auto px-4 py-6 text-center h-[20vh]">
        ...
        y: 0
                     ...
                     bg-clip-text text-transparent">Aayush Bhat Photography</motion.h1>
      ...
      <div className="w-full">
        <div className="container mx-auto px-4 mt-[10vh]">
          <div className="relative glass-card p-4 rounded-xl max-w-4xl mx-auto" onMouseEnter={() => setIsAutoPlaying(false)} onMouseLeave={() => setIsAutoPlaying(true)}>
            <button onClick={() => setCurrentImageIndex(prev => prev === 0 ? portfolioImages.length - 1 : prev - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-background/50 p-2 rounded-full
                       hover:bg-background/70 transition-colors">
              <ChevronLeft className="w-6 h-6 text-primary" />
            </button>
            <button onClick={() => setCurrentImageIndex(prev => (prev + 1) % portfolioImages.length)} className="absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-background/50 p-2 rounded-full
                       hover:bg-background/70 transition-colors">
            ...
            <div className="relative overflow-hidden rounded-lg">
              <AspectRatio ratio={16 / 9}>
                <AnimatePresence mode="wait">
                  <motion.img key={currentImageIndex} src={portfolioImages[currentImageIndex].src} alt={portfolioImages[currentImageIndex].title} className="absolute inset-0 w-full h-full object-cover" initial={{
                  opacity: 0,
                  scale: 1.1
                }} animate={{
                  opacity: 1,
            ...
            <div className="mt-4">
              <Progress value={(currentImageIndex + 1) / portfolioImages.length * 100} className="h-1" />
            </div>

            <div className="flex justify-center gap-2 mt-4">
              {portfolioImages.map((_, index) => <button key={index} onClick={() => setCurrentImageIndex(index)} className={`w-2 h-2 rounded-full transition-colors ${index === currentImageIndex ? "bg-primary" : "bg-primary/30"}`} />)}
            </div>
          </div>
        </div>
      </div>
